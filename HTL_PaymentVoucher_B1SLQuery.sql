...
SELECT        
    A.DocEntry,  
    CONCAT(ISNULL(N.BeginStr,''), ISNULL(A.DocNum,'')) AS PCNo, 
    A.TaxDate AS Date, 
    A.DocDate AS [Posting Date],
    A.U_HT_JobNo AS [JobNum],
    M."NAME" AS JobNo,
    P.PrcName AS LOB, 
    A.NumAtCard AS SuppliersRef, 
    A.CardName AS PayTo, 
    A.Address AS Dep_Address, 
    AC.AcctName AS Account, 
...
FROM    dbo.OPCH AS A LEFT OUTER JOIN
        ...
        dbo.OPRC AS P ON A.U_cost_center = P.PrcCode LEFT OUTER JOIN
        DBO.OPMG AS M ON A.U_HT_JobNo = M.AbsEntry
WHERE A.CANCELED = 'N'
